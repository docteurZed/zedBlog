<div>
            ...
            <div class="border border-gray-200 dark:border-gray-700 p-8 rounded-4xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white md:flex justify-between w-full">
                <div class="md:flex gap-4 mb-8 md:mb-0">
                    ...
                    <div class="col-span-3 m-3">
                        <h2 class="mb-3 text-4xl text-gray-900 dark:text-white font-bold">Auteurs</h2>
                        <p class="mb-4 font-normal text-gray-600 dark:text-gray-400 w-full md:w-[80%]">Tous les auteurs enrégistrés sont disponibles ici.</p>
                        <div class="flex gap-2 text-gray-700 dark:text-gray-200 items-center">
                            ...
                            <span class="font-semibold text-sm">{{ $authors->total() }} auteur{{ $authors->total() > 1 ? 's' : '' }}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="p-8 md:p-16">
        <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8">
            ...
            <a href="{{ route('author.detail', ['id' => $author->id, 'slug' => Str::slug($author->name)]) }}" class="relative group w-full min-h-64 border border-dashed border-gray-400 dark:border-gray-600 rounded-4xl p-8 flex items-center flex-col gap-2 cursor-pointer overflow-hidden">
                ...
                <div class="relative z-10 flex items-center flex-col gap-2">
                    ...
                    <h4 class="text-gray-900 dark:text-white font-bold text-lg">{{ $author->name }}</h4>
                </div>
            ...
            <div class="sm:col-span-2 md:col-span-3 lg:col-span-4 flex justify-center border border-gray-200 dark:border-gray-700 rounded-3xl group p-5">
                <p class="text-gray-600 dark:text-gray-400 font-semibold">Aucun auteur disponible</p>
            </div>
            @endforelse
        </div>
        @if ($authors->total() > $authors->perPage())
        <div class="my-12 flex justify-between items-center font-semibold text-gray-900 dark:text-white">
            ...
            <div>
                <button
                    ...
                    @class([
                        ...
                        'text-gray-400 cursor-not-allowed' => $authors->onFirstPage(),
                        'text-gray-900 dark:text-white' => !$authors->onFirstPage(),
                    ])
                    @disabled($authors->onFirstPage())
                >
                ...
                @for ($page = 1; $page <= $authors->lastPage(); $page++)
                    <button
                        ...
                        class="rounded-2xl px-4 py-3 transition ease-in-out duration-150 hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer
                            {{ $authors->currentPage() === $page ? 'bg-gray-900 text-white' : 'text-gray-900 dark:text-white' }}"
                    >
                        {{ $page }}
                    </button>
                @endfor
            ...
            <div>
                <button
                    ...
                    @class([
                        'rounded-2xl px-4 py-3 flex gap-2 items-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition ease-in-out duration-150',
                        'text-gray-400 cursor-not-allowed' => !$authors->hasMorePages(),
                        'text-gray-900 dark:text-white' => $authors->hasMorePages(),
                    ])
                    @disabled(!$authors->hasMorePages())
                >
                    <span>Suivant</span>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-6 h-6" viewBox="0 0 16 16">
