            ...
            <div class="border border-gray-200 dark:border-gray-700 p-8 rounded-4xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white md:flex justify-between w-full">
                <div class="md:flex gap-4 mb-8 md:mb-0">
                    ...
                    <div class="col-span-3 m-3">
                        <div class="mb-1">
                            <span class="bg-yellow-600 text-sm font-semibold text-white px-2 py-1 rounded-full">Catégorie</span>
                        </div>
                        <h2 class="mb-4 text-4xl text-gray-900 dark:text-white font-bold">{{ $category->name }}</h2>
                        <p class="font-normal text-gray-600 dark:text-gray-400 w-full md:w-[80%]">{!! $category->description !!}</p>
                        <div class="mt-4 flex gap-2 text-gray-700 dark:text-gray-200 items-center">
                            ...
                            <span class="font-semibold text-sm">{{ $category->posts->total() }} article{{ $category->posts->total() > 1 ? 's' : '' }}</span>
                        </div>
                    </div>
                </div>
                <div class="mt-8">
                    <div class="flex gap-2">
                        <button wire:click="toggleSubscription" type="button" class="border border-gray-400 dark:border-gray-600 rounded-full px-4 py-3 text-gray-900 dark:text-white flex gap-2 items-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 {{ $isSubscribed ? 'bg-gray-400 dark:bg-gray-600' : '' }} transition ease-in-out duration-150">
                            @if ($isSubscribed)
                            <span class="text-sm font-semibold">Suivi</span>
                            @else
    ...
    <div class="p-8 md:p-16">
        ...
        <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            @forelse ($posts as $post)
                @livewire('guest.article.detail', ['post' => $post, 'category' => $category], key($post->id))
            @empty
